...
router.post('/persionInfo',function(req,res,next) {
  ...
  if (!req.headers.token || req.headers.token.length == 0){
    ...
    if (req.body.type == 0){//.获取信息
      ...
      model.data = [[_info.iconUrl,_info.nickName,_info.sex],[_info.phoneNum,_info.emaill],[_info.birthday,_info.education,_info.endEducationTime,_info.workYears,_info.address]];
      model.code = 0;
    }else {//上传信息
